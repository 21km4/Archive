#include "crypto.h"
#include <stdint.h>

static const uint8_t SBox[] = {
	
};

static void XorBlock(const uint8_t* const in, const uint8_t* out) // Ensure that this is always 32 bit aligned
{
	uint8_t i;
	for (i = 0; i < AES_BLOCK_WORDS; i++)
	{
		((uint32_t*)out)[i] ^= ((uint32_t*)in)[i];
	}
}

#define ROR32(v, n)  ( (v) << (32 - n) | (v) >> n )
#define AddRoundKey(d, rk) XorBlock((const uint8_t*)rk, (const uint8_t*)d)

#define Mul2(word) (((word & 0x7f7f7f7f) << 1) ^ (((word & 0x80808080) >> 7) * 0x1b))
#define Mul3(word) (Mul2(word) ^ word)
#define Mul4(word) (Mul2(Mul2(word)))
#define Mul8(word) (Mul2(Mul2(Mul2(word))))
#define Mul9(word) (Mul8(word) ^ word)
#define MulB(word) (Mul8(word) ^ Mul3(word))
#define MulD(word) (Mul8(word) ^ Mul4(word) ^ word)
#define MulE(word) (Mul8(word) ^ Mul4(word) ^ Mul2(word))

#define F0(x, y, z)  ( ((x) & (y)) | (~(x) & (z)) )
#define F1(x, y, z)  ( ((x) & (y)) | ((x) & (z)) | ((y) & (z)) )

#define SI1(x)  ( ROR32(x, 2 ) ^ ROR32(x, 13) ^ ROR32(x, 22) )
#define SI2(x)  ( ROR32(x, 6 ) ^ ROR32(x, 11) ^ ROR32(x, 25) )
#define SI3(x)  ( ROR32(x, 7 ) ^ ROR32(x, 18) ^ ((x) >> 3 ) )
#define SI4(x)  ( ROR32(x, 17) ^ ROR32(x, 19) ^ ((x) >> 10) )

static void MixColumnsR(uint8_t* state)
{
	uint8_t i = 0;
	for (; i < AES_BLOCK_WORDS; i++)
	{
		uint32_t word = ((uint32_t*)state)[i];
		((uint32_t*)state)[i] = MulE(word) ^ ROR32(MulB(word), 8) ^ ROR32(MulD(word), 16) ^ ROR32(Mul9(word), 24);
	}
}


static uint32_t SubDword(uint32_t v)
{
	uint8_t* b = (uint8_t*)&v;
	uint8_t i = 0;

	for (; i < sizeof(uint32_t); i++) b[i] = SBox[b[i]];

	return v;
}

void AesInitKey(AesCtx* Ctx, const uint8_t* Key, int RijndaelKeyBytes)
{
	int RijndaelKeyDwords = RijndaelKeyBytes / sizeof(uint32_t);
	Ctx->rounds = (uint8_t)(RijndaelKeyDwords + 6);

	static const uint32_t RCon[] = {
		0x00000000, 0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
		0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000 };

	uint8_t i;
	uint32_t temp;

	memcpy(Ctx->Key, Key, RijndaelKeyBytes);

	for (i = (uint8_t)RijndaelKeyDwords; i < (Ctx->rounds + 1) << 2; i++)
	{
		temp = Ctx->Key[i - 1];

		if ((i % RijndaelKeyDwords) == 0) temp = SubDword(ROR32(temp, 24)) ^ RCon[i / RijndaelKeyDwords];

		Ctx->Key[i] = Ctx->Key[i - RijndaelKeyDwords] ^ temp;
	}
}

static void SubBytes(uint8_t* block)
{
	uint8_t i;
	for (i = 0; i < AES_BLOCK_BYTES; i++) block[i] = SBox[block[i]];
}


static void ShiftRows(uint8_t* state)
{
	uint8_t bIn[AES_BLOCK_BYTES];
	uint8_t i;

	memcpy(bIn, state, AES_BLOCK_BYTES);
	for (i = 0; i < AES_BLOCK_BYTES; i++)
	{
		state[i] = bIn[(i + ((i & 3) << 2)) & 0xf];
	}
}


static void MixColumns(uint8_t* state)
{
	uint8_t i = 0;
	for (; i < AES_BLOCK_WORDS; i++)
	{
		uint32_t word = ((uint32_t*)state)[i];
		((uint32_t*)state)[i] = Mul2(word) ^ ROR32(Mul3(word), 8) ^ ROR32(word, 16) ^ ROR32(word, 24);
	}
}


void AesEncryptBlock(const AesCtx* const Ctx, void* _block)
{
	uint8_t* block = (uint8_t*)_block;
	uint8_t i;
	for (i = 0;; i += 4)
	{
		AddRoundKey(block, &Ctx->Key[i]);
		SubBytes(block);
		ShiftRows(block);

		if (i >= (Ctx->rounds - 1) << 2) break;

		MixColumns(block);
	}

	AddRoundKey(block, &Ctx->Key[Ctx->rounds << 2]);
}

static const uint8_t SBoxR[] = {
	
};

#define GetSBoxR(x) SBoxR[x]

static void ShiftRowsR(uint8_t* state)
{
	uint8_t b[AES_BLOCK_BYTES];
	uint8_t i;

	memcpy(b, state, AES_BLOCK_BYTES);

	for (i = 0; i < AES_BLOCK_BYTES; i++) state[i] = b[(i - ((i & 0x3) << 2)) & 0xf];
}


static void SubBytesR(uint8_t* block)
{
	uint8_t i;

	for (i = 0; i < AES_BLOCK_BYTES; i++)
		block[i] = GetSBoxR(block[i]);
}

size_t AesEncryptCbc(const AesCtx* const Ctx, void* _iv, void* _data, size_t len)
{
	uint8_t* iv = (uint8_t*)_iv, * data = (uint8_t*)_data;

	// Pad up to blocksize inclusive
	size_t i;
	uint8_t pad = (~len & (AES_BLOCK_BYTES - 1)) + 1;
	memset(data + len, pad, pad);
	len += pad;

	if (!data) return len;

	if (iv) XorBlock(iv, data);
	AesEncryptBlock(Ctx, data);

	for (i = len - AES_BLOCK_BYTES; i; i -= AES_BLOCK_BYTES)
	{
		XorBlock(data, data + AES_BLOCK_BYTES);
		data += AES_BLOCK_BYTES;
		AesEncryptBlock(Ctx, data);
	}

	return len;
}

void AesDecryptBlock(const AesCtx* const Ctx, void* _block)
{
	uint8_t* block = (uint8_t*)_block;
	uint8_t i;

	AddRoundKey(block, &Ctx->Key[Ctx->rounds << 2]);

	for (i = (Ctx->rounds - 1) << 2;; i -= 4)
	{
		ShiftRowsR(block);
		SubBytesR(block);
		AddRoundKey(block, &Ctx->Key[i]);

		if (i == 0) break;

		MixColumnsR(block);
	}
}


size_t AesDecryptCbc(const AesCtx* const Ctx, void* _iv, void* _data, size_t len)
{
	uint8_t* iv = (uint8_t*)_iv, * data = (uint8_t*)_data;
	uint8_t* cc;

	for (cc = data + len - AES_BLOCK_BYTES; cc > data; cc -= AES_BLOCK_BYTES)
	{
		AesDecryptBlock(Ctx, cc);
		XorBlock(cc - AES_BLOCK_BYTES, cc);
	}

	AesDecryptBlock(Ctx, cc);
	if (iv) XorBlock(iv, cc);

	return len - *(data + len - 1);
}
